(function() {
    ...
    var elIdLoadFPSpin = document.getElementById('load-fp-spin');
    ...
    elsNavA.forEach((elNavA) => {
        elNavA.addEventListener('click', function(event) {toogleMenuBurger();} );
    });
    ...
    if (elIdFiltersButton !== null){
        ...
        app({
            appId: '5IKVY0BULC', //'NUMQWE06PC',
            apiKey: 'e3c22f5936c0f7947f4a61ecee808f70',// '3982cd10b416047781ed12a74200ee35',
...
*/
    ...
    var toggleElement = function(elem, target) {
    ...
    document.addEventListener('click', function(event) {
        ...
        if (!currentTarget.classList.contains('toggle')) return;
    ...
    function initAutocomplete() {
        autocomplete = new google.maps.places.Autocomplete(
            document.getElementById('route'), { types: ['address'] });
    ...
    routing: {
      router: instantsearch.routers.history({
        ...
        createURL({ qsModule, routeState, location }) {
          const urlParts = location.href.match(/^(.*?)\/rechercher/);
          ...
          return `${baseUrl}rechercher${queryString}`;
        },

        parseURL({ qsModule, location }) {
          const pathnameMatches = location.pathname.match(/rechercher\/(.*?)\/?$/);
          const { query = '', page, marques = [], carrosseries = [], modeles = [], options = [] } = qsModule.parse(
            location.search.slice(1)
          );
          // `qs` does not return an array when there's a single value.
          const allMarques = Array.isArray(marques)
            ? marques
            : [marques].filter(Boolean);
          const allCarrosseries = Array.isArray(carrosseries)
            ? carrosseries
            : [carrosseries].filter(Boolean);
        const allOptions = Array.isArray(options)
            ? options
            : [options].filter(Boolean);
          const allModeles = Array.isArray(modeles)
            ? modeles
            : [modeles].filter(Boolean);
